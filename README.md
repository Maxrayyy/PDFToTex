...
总项目保存 README、Compose、运行脚本、部署文档和流水线源码；`Lexoid` 保留独立 Git 仓库。运行数据位于本目录上一级的 `data/`，由 Docker 挂载使用，不纳入 Git。不要把 `.env`、模型缓存和转换数据提交到 Git。
...
flowchart TD
    ...
    I -->|成功| J[发布 TEX 和 PDF 供核对]
    I -->|失败| K[保留缓存和日志，队列暂停]
```

1. **识别**：默认 `RECOGNITION_OCR=none`，主模型为 `gpt-5.6-sol`，使用 `SOL_VISION_REASONING_EFFORT=none`。视觉模型直接生成逐页 TEX；字段注释保留在 TEX 源码中，坐标只作为内部辅助信息。
...
3. **字段协调**：`gpt-6-astra` 对准确坐标读取 480 DPI 局部裁片；缺失或整页坐标的待复核字段按页合并请求，并持久化限制整页调用次数（默认每页 2 次）。默认只自动复核日期、批号、数量等格式异常。手写/印刷值和勾选不确定等保留首次值与人工核对标记；原始识别 TEX 不被覆盖。调用明细仅写入日志，详见[整页复核重复消耗](docs/issues/reconcile-full-page-cost.md)。
...
6. **延后语义命名**：默认 `TEXOPT_SEMANTIC_NAMING=deferred`，先交付 TEX/PDF 与基础字段信息。命名不修改字段值或 TEX，也不重新编译。
...
实际工作区中，`Downloads/`、`data/` 与 `PDFToTex/` 平级。`Downloads/` 保存原始 PDF，`data/` 保存工作区、正式 TEX、队列和监控状态；两者都不属于 `PDFToTex` Git 范围。
